...
class Block {
    constructor(public pos: Vec, public size: Vec, public colour: string) {
        this.pos = pos;
        this.size = size;
        this.colour = colour;
    }
...
class Game {
    ...
    constructor() {
        ...
        for (let i = 0; i < config.blocksY; i++) {
            ...
            for (let j = 0; j < (config.blocksX - i % 2); j++) {
                this.blocks[i][j] = new Block(
                    new Vec(
                        j * config.blockWidth + (i % 2) * config.blockWidth / 2 + config.margin,
                        i * config.blockHeight + config.margin / 2),
                    new Vec(config.blockWidth, config.blockHeight),
                    "#f24dee"
                );
            }
        }
        ...
        this.bat = new Block(
            new Vec(
                config.gameWidth / 2 - config.batWidth / 2,
                this.canvas.height - config.batHeight - config.blockGap
            ),
            new Vec(config.batWidth, config.batHeight),
            "#4ec2f0"
        );
